...
public class ReturnStatusEffect extends ModStatusEffect {
	...
	public void applyInstantEffect(Entity source, Entity attacker, LivingEntity target, int amplifier, double d) {
		if (target instanceof ServerPlayerEntity) {
			...
			if (respawnPos != null) {
				PlayerEntity.findRespawnPosition((ServerWorld) target.world, respawnPos, player.getYaw(), player.isSpawnForced(), !ModConfig.INSTANCE.useAnchorChargesWithReturnPotion).ifPresent(v3d -> {
